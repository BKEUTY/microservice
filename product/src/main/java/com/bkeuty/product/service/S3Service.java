...
public class S3Service {
    ...
    private String variantImageFolder;
     ...
     *
     * @param productId the ID of the refund order (used as the sub-folder name)
     * @param images        list of multipart files to upload
     * @return list of publicly accessible S3 URLs for the uploaded files
     */
    public List<String> uploadProductImages(Integer productId, List<MultipartFile> images) {
        ...
        for (MultipartFile image : images) {
            ...
            String s3Key = String.format("%s/%d/%s_%s",
                    productImageFolder, productId, UUID.randomUUID(), originalFilename);
    ...
    public List<String> uploadVariantImages(Integer variantId, List<MultipartFile> images) {
        List<String> urls = new ArrayList<>();
        ...
        for (MultipartFile image : images) {
            ...
            // S3 key: refund-evident/{refundOrderId}/{uuid}_{filename}
            String s3Key = String.format("%s/%d/%s_%s",variantImageFolder
                    , variantId, UUID.randomUUID(), originalFilename);
