...
public class PromotionController {
    ...
    public PromotionController(PromotionFactory promotionFactory, AuthService authService,  PromotionService promotionService) {
        this.promotionFactory = promotionFactory;
        this.authService = authService;
        this.promotionService = promotionService;
    }
    @PostMapping
    public ResponseEntity<CreatePromotionResponse> createPromotion(@RequestHeader("Authorization") String bearerToken,@RequestBody CreatePromotionRequest request) {
    ...
    @PutMapping("/{id}")
    public ResponseEntity<CreatePromotionResponse> updatePromotion(@RequestHeader("Authorization") String bearerToken,@PathVariable Integer id,@RequestBody CreatePromotionRequest request) {
        TokenValidationResponseDto tokenValidationResponseDto = authService.validateToken(bearerToken);
        if (tokenValidationResponseDto.getUserId() == null || tokenValidationResponseDto.getUserRole() == null
                || !"admin".equals(tokenValidationResponseDto.getUserRole())) {
            return new ResponseEntity<>(HttpStatus.UNAUTHORIZED);
    ...
    @GetMapping
    public ResponseEntity<Page<PromotionResponseDto>> getPromotion(@RequestHeader("Authorization") String bearerToken,@RequestParam(defaultValue = "0") int page ) {
        TokenValidationResponseDto tokenValidationResponseDto = authService.validateToken(bearerToken);
        if (tokenValidationResponseDto.getUserId() == null || tokenValidationResponseDto.getUserRole() == null
                || !"admin".equals(tokenValidationResponseDto.getUserRole())) {
            return new ResponseEntity<>(HttpStatus.UNAUTHORIZED);
